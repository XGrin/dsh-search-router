      ...
      result: {
        ...
        value: {
          ...
          namespaces: [{
            ...
            revision: 1,
            value: { ...BASE, ...userLayer },
            secrets: ["exaApiKey", "tavilyApiKey", "braveApiKey"].map((field) => ({ path: [field], set: Object.hasOwn(userLayer, field) })),
          }],
        },
      },
    }),
...
check("auto mode with nothing configured → empty chain", state.explicit === false && state.chain.length === 0 && state.addable.length === 4, { chain: state.chain, addable: state.addable });
check(
...
check("adding commits order [exa]", state.chain.join() === "exa" && scopeWrites.at(-1)?.op === "set" && scopeWrites.at(-1)?.field === "order" && scopeWrites.at(-1)?.value === "exa", scopeWrites.at(-1));
await actions.addProvider("searxng");
await flush();
await actions.addProvider("tavily");
await flush();
state = face.hooks.searchRouterCard.getSnapshot();
check("chain reflects additions in order", state.chain.join() === "exa,searxng,tavily", state.chain);
check("addable shrinks to the remainder", state.addable.join() === "brave", state.addable);
...
await flush();
state = face.hooks.searchRouterCard.getSnapshot();
check("removing drops the provider and commits", state.chain.join() === "searxng,exa" && scopeWrites.at(-1)?.value === "searxng, exa", scopeWrites.at(-1));

/* removing the last card returns to automatic */
await actions.removeProvider("searxng");
await actions.removeProvider("exa");
await flush();
state = face.hooks.searchRouterCard.getSnapshot();
check("empty chain unsets order (back to auto)", state.chain.length === 0 && scopeWrites.at(-1)?.op === "unset" && scopeWrites.at(-1)?.field === "order", scopeWrites.at(-1));
...
check("stored key makes the provider auto-detected", state.chain.join() === "tavily" && state.explicit === false, state.chain);
...
await flush();
state = face.hooks.searchRouterCard.getSnapshot();
check("env credential alone also auto-detects the provider", state.chain.join() === "tavily", state.chain);
...
await actions.writeSearxngUrl("http://127.0.0.1:8888");
await flush();
state = face.hooks.searchRouterCard.getSnapshot();
...
check("auto chain now includes searxng after tavily", state.chain.join() === "tavily,searxng", state.chain);
await actions.writeSearxngUrl("");
await flush();
